...
max_dilation_per_kernels = [8, 16, 32, 64, 119]

cnt = 0
f = open('mini_max_dilation_per_kernel.csv','w',encoding='utf-8')
csv_writer = csv.writer(f)
csv_writer.writerow(['classifier_name', 'dataset_name', 'accuracy'])
for kernels in max_dilation_per_kernels:
    cnt = 0
    for file in sorted(files):
        cnt += 1
        acc = minirocket_acc(file, num_features=10000, max_dilations_per_kernel=kernels)
        print(str(kernels) + " " + str(cnt)+ " " + file + " " + str(acc))
        csv_writer.writerow([str(kernels), file, acc])

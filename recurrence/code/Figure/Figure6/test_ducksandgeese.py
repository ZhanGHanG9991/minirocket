...
def minirocket_arff_acc(dataset_name, sample_num):
    ...
    X_Y_training_df = X_Y_training_df.sample(n=sample_num, random_state=1, axis=1)
    ...
    X_Y_test_target = X_Y_test_df['target']
    X_Y_test_df = X_Y_test_df.drop(labels='target', axis=1)
    X_Y_test_df = X_Y_test_df.sample(n=sample_num, random_state=1, axis=1)
    X_Y_test_df['target'] = X_Y_test_target
...
def minirocket_arff_acc_no_sample(dataset_name):
...
print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' al' + ' ' + str(acc) + ' ' + str(t))
print("Finish")